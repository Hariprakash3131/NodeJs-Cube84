...
(() => {
  ...
  if (s2pin && !reduceMotion.matches) {
    ...
    const applyS2 = (p) => {
      ...
      labels.forEach((label, i) => {
        ...
        // Transform/opacity stay on the compositor. Animating blur here forces
        // a full repaint of the large pinned stage on every scroll frame.
        label.style.filter = "";
      });
    };
    ...
    const applyTr = (pt) => {
      ...
      tiles.forEach(({ el, seam, cfg }) => {
        ...
        // Avoid animated grayscale/invert on large clipped clones: it is much
        // more expensive than transforms and opacity, especially on mobile GPUs.
        if (cfg.inv) el.style.filter = "";
  ...
  if (rbSection && typeof heroContent === "object" && heroContent.bigRobot) {
    ...
    const onScroll = () => {
      ...
      // Do not wake this animation loop while another chapter is on screen.
      if (scrollY < pinTop - window.innerHeight || scrollY > pinTop + pinRange + window.innerHeight) return;
      jpTarget = clamp01((scrollY - pinTop) / pinRange);
      kick();
    };
  ...
  if (edSection && typeof heroContent === "object" && heroContent.editorial) {
    ...
    const onScroll = () => {
      const scrollY = window.scrollY;
      // Rendering every skill item while this section is far off-screen makes
      // the rest of the page compete with this chapter for animation frames.
      if (scrollY < pinTop - window.innerHeight || scrollY > pinTop + pinRange + window.innerHeight) return;
      pTarget = clamp01((scrollY - pinTop) / pinRange);
      kick();
    };
